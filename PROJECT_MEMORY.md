...
- **CTA strategy (2026-09-22):** Keep one primary general discovery entry in the main header navigation: `navigation.discover` (Discover / აღმოაჩინე / Найти) → `/discover`, with the same entry in the mobile menu. Keep professional registration in the upper-right header/mobile menu → `/register?role=barber`. Header direct Book is removed site-wide. Hero has no Find a Barber/Salon buttons. How It Works keeps its three localized explanatory steps with no link/button. The entire bottom barber-registration poster is removed. The homepage footer also omits generic Discover and direct booking prompts; other-route footers retain their previous links. Lower filter form, availability/style shortcuts, directory links, cards, ratings, sorting, pricing, routes and booking logic remain unchanged.
- **Cleanup/layout:** Removed six dead translation keys and orphaned CTA/banner CSS. Desktop/tablet hero minimum heights reduced by 45–55px with copy/location spacing tightened; artwork/motion behavior unchanged. How It Works columns vertically align, and homepage footer has no redundant top margin. Registration link remains visible at tablet widths above 800px; below that it lives in the existing mobile menu.

- **Components:** `src/components/barber-hero.tsx` and `barber-hero.css`; `app/page.tsx` composes the hero, lower marketplace and informational How It Works section. Copy and footer captions are ordinary localized HTML; hero discovery/salon CTAs were removed on 2026-09-22. Desktop has left copy and right cinematic artwork; mobile stacks text above a portrait crop. No canvas, mannequin selectors, hotspots, rotation or Photo View state remain. The noninteractive artwork is `aria-hidden` and does not intercept clicks/touch scrolling.
...
- **Navigation/state:** The hero is informational/decorative and contains no buttons or links. It does not create or clear inspiration. Existing `chair.style.v1` data is retained for earlier submitted discovery/booking briefs; `setHairStyle` and `clearSelection` remain used outside the hero. Removed only unused `setBeardStyle`, `submitSelection` and former hero URL helper. Preserve existing beard/submitted fields and the six shared beard labels. New visitors cannot create a mannequin brief because that UI is retired.
...
- **Lower search preservation:** All 31 lower home-filter CSS rules/declarations/media contexts were extracted unchanged into `premium-home-filter.css`, imported by `premium-filter-bar.tsx`. The September 22 cleanup leaves lower search, shortcuts and shop/style/barber JSX unchanged through the beginning of How It Works. Language selector TSX/CSS and all three flag SVGs are unchanged; `.style-hero` remains solely a compatible home-theme hook for that CSS.
...
- Premium dark homepage with generated barber-chair/tool artwork, seamless CSS camera/light motion, responsive local images, static recovery, reduced motion, offscreen pause and branding copy without discovery/salon CTAs. The mannequin direction is fully retired.
...
- **Status:** **Live and verified on 2026-09-22.** Pages source is GitHub Actions (`build_type: workflow`), HTTPS enforced, repository public as explicitly authorized. Homepage CTA release `d4095a555bba9c439340b72b42a5dc87878dcc4a` passed build/deploy in [Actions run 35720062139](https://github.com/CHAIR-DOT/chair-barber-marketplace/actions/runs/35720062139). Live browser confirms removed CTAs, retained explanatory/lower content, working header Discover and barber registration, Georgian preference, loaded images and no warning/error logs. Initial deployment was `e4729e7` on September 16. Use the latest Actions run and Git HEAD for later documentation-only pushes.
...
**Homepage CTA cleanup, 2026-09-22 (fresh local checks):** 30/30 tests and TypeScript pass. Normal and Pages builds each generate 57 pages. Static HTTP checks under the repository base path pass 55 valid routes, 5 expected unknown-route 404s and 31 images. All **15 cases (KA/EN/RU × 375/430/768/1024/1440px)** pass: no horizontal overflow/header overlap, removed elements absent, informational steps retained, zero gap between How It Works and footer, and 3 shop/6 style/4 barber cards remain. Desktop/tablet header has one visible Discover and one professional-registration entry; mobile menus contain their equivalents and no booking link. Actual language switching works, with Georgian as the isolated first-visit default. Screenshots reviewed for English desktop/mobile hero and How/footer, and Russian tablet/mobile hero. Header Discover, desktop/mobile professional registration, native Vake search (4 barbers / 2 shops), Giorgi profile and booking preselection (shop-1/barber-1/haircut, 45 minutes/₾35) work. Off-home footer discovery/booking links remain. Clean QA browser has no warning/error logs. No storage reset, account, booking or product record created. Source audit confirms unchanged routes, marketplace/provider/booking/filter/card logic, assets, dependency/config/workflow files. Detailed evidence is in `VERIFICATION.md`. Release `d4095a5` pushed successfully with local/upstream hashes equal; Actions build/deploy passed and live Georgian homepage/navigation checks confirm the cleanup with no missing images, tested overflow or warning/error logs. Normal localhost remains Georgian, with the previously saved Giorgi favorite and root form paths. Temporary static server stopped; viewport override reset; local/public previews remain available.
...
### 2026-09-22 — Homepage CTA cleanup

- User requested focused removal of repeated discovery, booking and professional-registration entry points without redesign or product changes. Audited complete homepage/header/footer and all three catalogs before editing; initial Git tree clean on established main.
- Removed header Book, both hero links, How It Works CTA and bottom registration poster. Homepage footer generic Discover/booking prompts also removed after audit; contextual lower search/directory/card links retained. Kept header Discover and professional registration; fixed the latter being hidden between 801–1100px.
- Removed six unused catalog keys/orphaned styles; adjusted hero spacing and footer transition. Updated current README, design/hero guide and memory descriptions. Fresh 30 tests, TypeScript and both normal/Pages builds pass (57 pages each). All 15 requested language/width cases pass; retained discovery, desktop/mobile registration, native Vake search and preselected profile booking work. Local static HTTP checks pass 55 routes, 5 expected 404s and 31 images. Implementation committed/pushed as `d4095a5`; Actions build/deploy and live homepage/retained navigation checks passed. Normal localhost remains Georgian with the saved Giorgi favorite. No data was reset. Verification docs record the release; future work can resume from the next requested frontend task.
...
**2026-09-22 (Asia/Tbilisi)** — Homepage CTA cleanup is complete, committed/pushed as `d4095a5` and verified on the public Pages site. Fresh tests, typecheck, normal/Pages builds, 15 responsive/locale cases and retained navigation/booking preselection pass. This handoff update records the successful release; use Git HEAD/upstream and the latest Actions run for documentation-only follow-ups. Temporary 3015 server stopped and viewport reset. A working local preview tab on 3000 and the public homepage are retained; the prior unreachable local tab was not modified. Existing local Georgian preference and Giorgi favorite remain. Exact next action: read memory/status and continue the next user-requested frontend change, preserving the simplified CTA strategy, routes/data and deployment workflow. No blocker or required application work remains from this task.
