...
**2026-09-22 (Asia/Tbilisi), release checkpoint** — Homepage CTA cleanup and local verification are complete. Exact next action: review/stage the scoped source, tests and docs, commit as `refactor: simplify homepage calls to action`, push main, verify Actions/public site, record the release result, then close the temporary 3015 QA server/tab and reset the viewport. The normal development preview on 3000 has been restarted. No routes, data, booking logic or storage were changed. Root owns this memory during parallel work.
