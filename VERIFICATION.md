...
### Release and final preservation checks

- Implementation commit **`d4095a555bba9c439340b72b42a5dc87878dcc4a`** (`refactor: simplify homepage calls to action`) pushed to existing `main`; local and origin/main hashes matched and tree was clean. [Actions run 35720062139](https://github.com/CHAIR-DOT/chair-barber-marketplace/actions/runs/35720062139) passed build and deployment.
- Live Georgian homepage at [the public URL](https://chair-dot.github.io/chair-barber-marketplace/) shows zero hero/header/home direct-booking links, no bottom registration banner, three informational steps, the home-specific footer and preserved lower sections. Header Discover opens discovery; professional registration opens the barber-role form. No missing images, observed overflow or warning/error logs. Returned the public tab to homepage.
- Normal local preview on 3000 was restarted and verified through a working tab: root `/discover` form action, Georgian preference, zero removed hero actions and the existing saved Giorgi favorite. No data reset or new booking/account/product record. The old unreachable local tab could not be selected by the browser tool and was left untouched; the working QA tab is retained as the local preview. Temporary static server on 3015 stopped; viewport overrides reset.
- Existing GitHub runner/action runtime migration annotations did not prevent successful build/deployment. Physical phones, formal accessibility and historical motion/failure tests remain outside this focused cleanup task.
